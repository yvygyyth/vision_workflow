...
def _probe(ctx, image: str, *, timeout: float = 0.8) -> bool:
    ...
    return find(image, timeout=timeout, interval=0.3, **_vision(ctx)).ok
...
def _setting_visible(ctx, *, timeout: float = 0.8) -> bool:
    ...
    return find(_SETTING, timeout=timeout, threshold=0.8, **_vision(ctx)).ok
...
def wait_game_start(ctx) -> Result:
    ...
    while time.monotonic() < deadline:
        ...
        sixes = find_all(_SIX, threshold=0.8, max_count=1, **_vision(ctx))
...
def pick_all_sixes(ctx) -> Result:
    ...
    result = find_all(_SIX, threshold=0.8, max_count=_MAX_SIX, **_vision(ctx))
...
def click_ok_if_any(ctx) -> Result:
    ...
    result = find(_OK, timeout=1.0, threshold=0.8, **_vision(ctx))
...
def click_next_step_if_any(ctx) -> Result:
    ...
    for _ in range(5):
        ...
        result = find(_NEXT_STEP, timeout=1.2, threshold=0.8, **_vision(ctx))
