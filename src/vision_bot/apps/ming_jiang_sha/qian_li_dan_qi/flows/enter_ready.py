...
from vision_bot.apps.ming_jiang_sha.qian_li_dan_qi.signals import ENTER_DETECT, ensure_registry, snap_center, snap_found
...
def try_start(ctx) -> Result:
    ...
    if c is None:
        ctx.goto(qmod(_FLOW, "recover"))
        return Result.success()
    Mouse().move(*c).click().sleep(0.5).perform()
    ctx.goto(qmod(_FLOW, "check_done"))
    return Result.success()
