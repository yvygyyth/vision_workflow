...
def build() -> Flow:
    return flow(
        ...
        children=[
            mod(
                "fee_day.zhan_yi_store.entry",
                "打开入口",
                lambda ctx: do_click(ctx, f"{_DIR}/entry.png", f"{_DIR}/entry2.png", timeout=5.0),
            ),
            mod("fee_day.zhan_yi_store.open_store", "打开商店", lambda ctx: do_click(ctx, f"{_DIR}/zhan-yi-store.png")),
            ...
            mod("fee_day.zhan_yi_store.scroll", "滚动", lambda ctx: scroll_center(ctx, -120, times=10)),
