...
class MainWindow(ctk.CTk):
    ...
    def _start(self, *, dry_run: bool) -> None:
        ...
        self.status.set_status("运行中…" if not dry_run else "干跑中…")
        self.logs.append(f"—— 开始 {'干跑' if dry_run else '运行'}：{target} ——")
        try:
            self.worker.start(RunRequest(target=target, dry_run=dry_run))
    ...
    def _apply_finish(
        ...
        self.logs.append(f"结果 success={result.success} | {result.feedback or result.message}")
